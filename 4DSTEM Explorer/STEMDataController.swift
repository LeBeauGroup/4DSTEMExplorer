...
class STEMDataController: NSObject {
    ...
    func openFile(url: URL) throws {
        ...
        if isTIFF {
            ...
            for (key, _) in imageList {
                ...
                if let name = imageDict["Name"] as? String, name == "Diffraction SI" {
                    tagGroup = key
                    break
                }
            }
            ...
            if let dtypeNumber = data["datatype"] as? Int{
                ...
                case 5: dataType = .uint32
                case 8: dataType = .bool
                case 10: dataType = .uint8
        ...
        // Validate dimensions before reading
        ...
        guard totalImages > 0, totalPatternPixels > 0 else {
            throw FileReadError.emptyDataset
        }

        // Read in a background work item (no polling loops)
        let nc = NotificationCenter.default
        dwi?.cancel()
        dwi = DispatchWorkItem { [weak self] in
            guard let self = self else { return }
            self.openFileHandle(url: url)
            guard let fh = self.fh else {
            ...
            do {
                try fh.seek(toOffset: firstImageOffset)
            } catch {
                DispatchQueue.main.async {
                    self.progressdelegate?.cancel(self)
                }
            ...
            let elementSize = dataType.elementSize
            // Use read(upToCount:) and treat short reads as EOF
            for batchIndex in 0..<( (totalImages + batchSize - 1) / batchSize ) {
                ...
                let batchData = fh.readData(ofLength: readSize) // single read; if short, we process what we got
                if batchData.isEmpty {
                    // EOF reached unexpectedly; stop cleanly
                    break
                }
                // If short read, adjust imagesInBatchDown
                let fullImageBytes = totalPatternPixels * elementSize
                let imagesRead = max(0, min(imagesInBatch, batchData.count / fullImageBytes))
                if imagesRead == 0 {
                    break
                }

                let count = imagesRead * totalPatternPixels
                self.convertToFloat(dataType: dataType, sourceData: batchData.prefix(imagesRead * fullImageBytes), destinationBuffer: floatTempBuffer, count: count)

                for img in 0..<imagesRead {
            ...
            DispatchQueue.main.async {
                // Even if cancelled or short-read, publish completion so UI can proceed.
        ...
        if let work = dwi {
            ...
            throw FileReadError.unreadableFile
        }
    }
    ...
    func averagePattern(rect:NSRect)->Matrix{
        ...
        if starti > endi{ strideDirectioni = -1 }
        if startj > endj{ strideDirectionj = -1 }
        var patternCount = 0
