...
public class StringDataset: HDF5Dataset {
    ...
    /// Read string data using an optional file HDF5Dataspace
    public func read(fileSpace: HDF5Dataspace? = nil) throws -> [String] {
    ...
    func readVariableLength(fileSpace: HDF5Dataspace? = nil) throws -> [String] {
        let count: Int
        ...
        let type = HDF5Datatype.createString()
        var data = [UnsafePointer<CChar>?](repeating: nil, count: count)
        let memspace = HDF5Dataspace(dims: [count])
    ...
    func readFixedLength(fileSpace: HDF5Dataspace? = nil) throws -> [String] {
        let count: Int
        if let fileSpace = fileSpace {
            count = fileSpace.selectionSize
        } else {
        ...
        let type = HDF5Datatype.createString(size: paddedStringSize)
    ...
    /// Write string data using an optional file HDF5Dataspace
    ///
    /// - precondition: The `selectionSize` of the file HDF5Dataspace is equal to `strings.count`
    public func write(_ strings: [String], fileSpace: HDF5Dataspace? = nil) throws {
        let size: Int
        if let fileSpace = fileSpace {
        ...
        precondition(strings.count == size, "Data size doesn't match HDF5Dataspace dimensions")
        ...
        let memspace = HDF5Dataspace(dims: [size])
        let type = HDF5Datatype.createString()
...
// MARK: HDF5GroupType extension for StringDataset

extension HDF5GroupType {
    /// Create a StringDataset
    public func createStringDataset(_ name: String, dataspace: HDF5Dataspace) -> StringDataset? {
        guard let datatype = HDF5Datatype(type: String.self) else {
            return nil
        }
    ...
    public func createStringDataset(_ name: String, dataspace: HDF5Dataspace, chunkDimensions: [Int]) -> StringDataset? {
        guard let datatype = HDF5Datatype(type: String.self) else {
            return nil
        }
