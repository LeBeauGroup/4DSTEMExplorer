...
public class DoubleDataset: HDF5Dataset {
    ...
    public func read(memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws -> [Double] {
        let size: Int
    ...
    public func read(into pointer: UnsafeMutablePointer<Double>, memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
    ...
    public func write(_ data: [Double], memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
        let size: Int
        if let memspace = memSpace {
            size = memspace.size
        } else if let filespace = fileSpace {
    ...
    ///
    /// - precondition: The `selectionSize` of the memory HDF5Dataspace is the same as for the file HDF5Dataspace
    public func write(from pointer: UnsafePointer<Double>, memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
        try super.write(from: pointer, type: .double, memSpace: memSpace, fileSpace: fileSpace)
    }
}


// MARK: HDF5GroupType extension for DoubleDataset

extension HDF5GroupType {
    /// Create a DoubleDataset
    public func createDoubleDataset(_ name: String, dataspace: HDF5Dataspace) -> DoubleDataset? {
        guard let datatype = HDF5Datatype(type: Double.self) else {
            return nil
        }
    ...
    public func createDoubleDataset(_ name: String, dataspace: HDF5Dataspace, chunkDimensions: [Int]) -> DoubleDataset? {
        guard let datatype = HDF5Datatype(type: Double.self) else {
            return nil
        }
    ...
    /// Create a Double HDF5Dataset and write data
