...
public class HDF5Dataspace {
    ...
    init(id: hid_t) {
        self.id = id
        guard id >= 0 else {
            fatalError("Failed to create HDF5Dataspace")
        }
        selectionDims = []
        selectionDims = dims
    }
    ...
    deinit {
        ...
        assert(status >= 0, "Failed to close HDF5Dataspace")
    }

    public internal(set) var selectionDims: [Int]

    /// Create a HDF5Dataspace
    ...
    /// Create a HDF5Dataspace for use in a chunked HDF5Dataset. No component of `maxDims` should be less than the corresponding element of `dims`. Elements of `maxDims` can have a value of -1, those dimension will have an unlimited size.
    ...
    public convenience init(_ space: HDF5Dataspace) {
        self.init(id: H5Scopy(space.id))
    }

    /// The total number of elements in the HDF5Dataspace
    public var size: Int {
        let result = H5Sget_simple_extent_npoints(id)
        guard result >= 0 else {
            fatalError("Failed to get HDF5Dataspace size")
        }
        return Int(result)
    }

    /// The size of each dimension in the HDF5Dataspace
    public var dims: [Int] {
        ...
        dims.withUnsafeMutableBufferPointer { pointer in
            guard H5Sget_simple_extent_dims(id, pointer.baseAddress, nil) >= 0 else {
                fatalError("Coulnd't get the dimensons of the HDF5Dataspace")
            }
        }
        return dims.map({ Int(hssize_t(bitPattern: $0)) })
    }

    /// The maximum size of each dimension in the HDF5Dataspace
