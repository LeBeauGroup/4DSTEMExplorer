...
public class IntDataset: HDF5Dataset {
    ...
    public func read(memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws -> [Int] {
        let size: Int
    ...
    public func read(into pointer: UnsafeMutablePointer<Int>, memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
    ...
    public func write(_ data: [Int], memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
        let size: Int
        if let memspace = memSpace {
            size = memspace.size
        } else if let filespace = fileSpace {
    ...
    ///
    /// - precondition: The `selectionSize` of the memory HDF5Dataspace is the same as for the file HDF5Dataspace
    public func write(from pointer: UnsafePointer<Int>, memSpace: HDF5Dataspace? = nil, fileSpace: HDF5Dataspace? = nil) throws {
        try super.write(from: pointer, type: .int, memSpace: memSpace, fileSpace: fileSpace)
    }
}


// MARK: HDF5GroupType extension for IntDataset

extension HDF5GroupType {
    /// Create a IntDataset
    public func createIntDataset(_ name: String, dataspace: HDF5Dataspace) -> IntDataset? {
        guard let datatype = HDF5Datatype(type: Int.self) else {
            return nil
        }
    ...
    public func createIntDataset(_ name: String, dataspace: HDF5Dataspace, chunkDimensions: [Int]) -> IntDataset? {
        guard let datatype = HDF5Datatype(type: Int.self) else {
            return nil
        }
    ...
    /// Create an Int HDF5Dataset and write data
